...
app.post('/api/transactions', authenticateJWT, requireKYC, async (req, res): Promise<any> => {
   try {
      const {
         ...
         itemDescription,
         agreedPrice,
         sellerEmail // if the buyer is creating it, who is seller?
      } = req.body;
      ...
      const serviceFee = basePrice * 0.05; // 5% flat fee
      const totalAmount = basePrice + serviceFee;
      ...
      const tradeRes = await prisma.$transaction(async (tx) => {
         const trade = await tx.transaction.create({
            data: {
               buyer_id: buyerId,
               seller_id: sellerId,
               item_type: itemCategory + ' - ' + itemDescription,
               game_type: itemCategory,
               agreed_price: basePrice,
               service_fee: serviceFee,
               total_amount: totalAmount,
               status: 'pending_invite', // Escrow Room not yet created
               inspection_hours: 24,
            }
         });
...
app.put('/api/transactions/:id/progress', authenticateJWT, async (req, res): Promise<any> => {
   try {
      const tradeId = parseInt(req.params.id as string);
      const { action } = req.body; // 'PAY', 'DELIVER', 'APPROVE'
         ...
         await prisma.$transaction(async (tx) => {
            ...
            await tx.message.create({
               data: {
                  ...
                  message_text: `[SYSTEM LOG] The Buyer has securely deposited ₱${Number(trade.total_amount).toLocaleString()} into the Midly Smart Vault. Seller, please proceed to Handover the item.`,
                  is_system_generated: true,
                  risk_level: 'Safe'
               }
            });
