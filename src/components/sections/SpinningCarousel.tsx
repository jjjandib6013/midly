...
export interface SpinningCarouselHandle {
  triggerSpeedBurst: (durationMs?: number, speed?: number) => void;
}
...
const SpinningCarousel = forwardRef<SpinningCarouselHandle, SpinningCarouselProps>(
  function SpinningCarousel({ entranceDelay = 0.2 }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      triggerSpeedBurst: (durationMs = 1500, speed = 50) => {
        ...
        burstTimeoutRef.current = window.setTimeout(() => {
          const burstDecay = () => {
            if (Math.abs(velocityRef.current) > 0.3) {
              ...
              requestAnimationFrame(burstDecay);
            }
          };
          requestAnimationFrame(burstDecay);
        }, durationMs);
    ...
    useEffect(() => {
      ...
      gsap.to(entranceYRef, { current: 0, duration: 2, ease: "power3.out", delay: entranceDelay });
      gsap.to(entranceOpacityRef, { current: 1, duration: 1.5, ease: "power2.inOut", delay: entranceDelay });

    }, [mounted, startSpin, entranceDelay]);
    ...
    if (!mounted) return <div style={{ height: "34vh" }} />;

    return (
      <div
        className="relative w-full max-w-[1200px] mx-auto select-none cursor-grab active:cursor-grabbing"
        style={{ height: "34vh", perspective: "1200px" }}
      ...
      >
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/3 w-[760px] h-[560px] bg-primary/[0.04] rounded-full blur-[200px] pointer-events-none" />
        <div className="absolute top-1/3 right-[6%] w-[420px] h-[420px] bg-primary/[0.07] rounded-full blur-[160px] pointer-events-none" />
        ...
        >
          {CARDS.map((card, idx) => {
            ...
            const cardW = "clamp(140px, 12vw, 210px)";
            const cardH = "clamp(195px, 16.5vw, 285px)";
            ...
            return (
              ...
              >
                ...
                {/* Icon */}
                <div
                  className="w-16 h-16 rounded-xl flex items-center justify-center mb-5 border"
                  style={{
                ...
                >
                  <Icon className="w-8 h-8" style={{ color: card.accent }} />
                </div>

                <h4 className="text-base sm:text-lg font-bold text-white mb-1.5 tracking-tight px-5">{card.title}</h4>
                <p className="text-[10px] sm:text-xs text-[#8892b0] font-medium max-w-[180px] px-4">{card.description}</p>
              </div>
            );
          })}
        </div>
